...
确定要查找的区间到底是：
- 🔸 **左闭右开** `[left, right)` - Python 风格
- 🔹 **左闭右闭** `[left, right]` - C++ 风格（推荐）

> 💡 这就是不变量，「二分查找的循环中，坚持循环不变量的原则」
...
### 💡 核心思想
通过一个**快指针**和**慢指针**在一个 `for` 循环下完成两个 `for` 循环的工作，将 **O(n²)** 优化为 **O(n)**。

> 🎯 **本质**：用空间换时间的思想，通过两个指针在不同的位置协同工作，避免嵌套循环
...
- 例如：每隔 k 个字符处理时，可以使用 `for (int i = 0; i < n; i += k)` 的步长控制

**示例**：
```cpp
// 每隔 2k 个字符处理前 k 个字符
for (int i = 0; i < s.length(); i += 2*k) {
    // 处理 [i, min(i+k, n)) 范围的字符
